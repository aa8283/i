...
function UILibrary:CreateWindow(title, width, height)
    ...
    -- Load saved position/size (if any)
    local savedData = getgenv().NiniUISavedData and HttpService:JSONDecode(getgenv().NiniUISavedData) or {}
    ...
    OuterFrame.BackgroundTransparency = 1 -- Start transparent
    ...
    OuterFrame.Parent = ScreenGui
    OuterFrame.Size = UDim2.new(0, windowWidth * 0.8, 0, windowHeight * 0.8) -- Start smaller
    ...
    TitleBar.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            local clampedX = math.clamp(newPos.X.Offset, -windowWidth/2, viewportSize.X - windowWidth/2)
            local clampedY = math.clamp(newPos.Y.Offset, -windowHeight/2, viewportSize.Y - windowHeight/2)
    ...
    TabContainer.Size = UDim2.new(0, 120, 1, -65) -- Adjusted for search bar
    ...
    local function CreateTab(tabName)
        ...
        SearchBar:GetPropertyChangedSignal("Text"):Connect(function()
            ...
            if searchText == "" then
                ...
                TabButton.Visible = tabName:lower():find(searchText) ~= nil
            end
            TabContainer.CanvasSize = UDim2.new(0, 0, 0, TabListLayout.AbsoluteContentSize.Y + 10)
        end)
